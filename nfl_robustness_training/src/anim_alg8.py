"""
Animation for alg8 — dynamic symbolic horizon with chunked cost and pow_vol.

Same visual style as anim_alg6. Key behavioural differences vs alg6:
  - symbolic_step receives budget.chunked_cost(max_symbolic_horizon) as first arg
    (chunk_time) instead of the raw horizon, and budget.max_affordable_symbolic()
    as timestep_length
  - optimized_step passes pow_vol=2 to ext_optimizer.get_strategy()
  - try_extend likewise uses budget.chunked_cost(max_symbolic_horizon)
...
from alg8_optimization_w_custom_cost import (
...
def try_extend_anim(frames, tester, validated_until, max_time, budget,
    ...
    """
    try_extend (alg8 variant) with frame captures.
    Uses budget.chunked_cost(max_symbolic_horizon) as chunk_time.
    """
    ...
    while validated_until_inner < scan_ceiling:
        ...
        # alg8: chunk_time = budget.chunked_cost(max_symbolic_horizon)
        job = VerificationTask(symbolic_start=validated_until_inner,
                               conflict_time=conflict_time)
        sym_start = job.symbolic_start
        job, result_s = symbolic_step(
            tester, job,
            budget.chunked_cost(max_symbolic_horizon),   # chunk_time  (alg8)
            budget.max_affordable_symbolic()              # timestep_length
        )
...
def optimized_step_anim(frames, tester, validated_until, max_time, budget,
                        ...
                        ext_optimizer):
    """
    optimized_step (alg8 variant) with frame captures.
    Passes pow_vol=2 to get_strategy(); uses chunked_cost for symbolic_step.
    All frames tagged 'optimizer'.
    """
    ...
    push(frames, tester, current_timestep,
         ...
         f"cur_vol={current_vol:.3f}  ver_vol={verified_vol:.3f}  pow_vol=2",
         "optimizer")
    ...
    else:  # symbolic — no budget cap on span
...
while current_timestep < MAX_TIME:
    ...
    push(frames, tester, current_timestep,
         f"t={current_timestep}  vu={validated_until}  margin={safety_margin}  "
         f"dyn_h={dynamic_symbolic_horizon}  [{mode}]",
         "info")

    # ── Phase 1: carry-over ────────────────────────────────────────────
    if pending_job is not None:
        sym_from = pending_job.symbolic_start
        # alg8: chunk_time = budget.chunked_cost(dynamic_symbolic_horizon)
        pending_job, result = symbolic_step(
            tester, pending_job,
            budget.chunked_cost(dynamic_symbolic_horizon),   # chunk_time
            budget.max_affordable_symbolic()                  # timestep_length
        )
        push(frames, tester, current_timestep,
    ...
    # ── Phase 2: optimized or baseline ────────────────────────────────
        ...
        if safety_margin >= MIN_SAFE_HORIZON:
            # ── OPTIMIZED: loop until budget gone, horizon maxed, or collision ──
            ...
            if not collision:
                ...
                pending_job, result_s = symbolic_step(
                    tester, pending_job,
                    budget.chunked_cost(dynamic_symbolic_horizon),   # chunk_time
                    budget.max_affordable_symbolic()                  # timestep_length
                )
...
# ══════════════════════════════════════════════
# Visual scheme (identical to anim_alg6)
#
# RSOA boxes are rendered with different BORDER styles depending on origin:
#   baseline  → thin dashed border
#   optimizer → thick solid border + hatching
#   info      → no extra decoration
#
# Fill colour encodes calc type (concrete/symbolic/empirical).
# ══════════════════════════════════════════════
...
def update(frame_idx):
    ...
    # Mode badge
    if origin == "optimizer":
        badge_color, badge_text = "#22c55e", "● OPT (pow_vol=2)"
...
legend_els = [
    ...
    Patch(facecolor="#888", alpha=0.3, edgecolor="#22c55e",
          linewidth=2.0, hatch="//", label="Optimizer calc (pow_vol=2)"),
...
out_path = "alg8_optimization.gif"
