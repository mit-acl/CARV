"""
Adaptive MPC algorithm — acados inner solver (alg11)

Same verification + safety-filter pipeline as alg10_mpc_w_custom_cost.py,
but the MPC safety filter uses acados (SQP + HPIPM) instead of do_mpc /
IPOPT.

Obstacles are in the form: [center_x, center_y, radius]

To call with a specific np seed:
    python alg11_mpc_acados.py <seed_number>
"""
