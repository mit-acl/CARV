...
from multi_obj_opt.strategies import ExtensionOptimizer, TimeEstimator
import numpy as np
...
def symbolic_step(tester, job: VerificationTask, chunk_size: int, timestep_length, time_estimator):
    """
    Advance a VerificationTask by one chunk.
    Returns (updated_job, result_dict | None).
    result_dict is non-None only when the job is complete after this step.
    ...
    chunk_time = time_estimator.symbolic_time_from_horizon(chunk_size)
    if (chunk_time - job.time_invested <= timestep_length):
        verify_end = min(job.conflict_time, job.symbolic_start + chunk_size)
        result = tester.symbolic(job.symbolic_start, verify_end)
        job.symbolic_start = verify_end
        job.time_invested = 0
...
def try_extend(tester, validated_until, max_time, budget, max_symbolic_horizon,
               current_timestep, min_lookahead, time_estimator, safe_horizon_ceiling=None):
    """
    ...
    while validated_until < scan_ceiling:
        ...
        job, result = symbolic_step(tester, job, max_symbolic_horizon, budget.max_affordable_symbolic(), time_estimator)
...
def optimized_step(tester, validated_until, max_time, budget, max_symbolic_horizon,
                   current_timestep, min_lookahead, ext_optimizer, time_estimator):
    """
...
def get_dynamic_symbolic_horizon(safety_margin, time_estimator, max_symbolic_horizon=10, symbolic_buffer=5):
    print(f"getting dynamic horizon: {(safety_margin, symbolic_buffer)=}")
    return min(max_symbolic_horizon, time_estimator.symbolic_horizon_from_time(safety_margin - symbolic_buffer))
...
def test1():
    ...
    ext_optimizer.set_timing_params(default_time_params)
    time_estimator = TimeEstimator()
    time_estimator.set_timing_params(default_time_params)
    ...
    while current_timestep < MAX_TIME:
        ...
        if pending_job is not None:
            ...
            pending_job, result = symbolic_step(tester, pending_job, dynamic_symbolic_horizon, budget.max_affordable_symbolic(), time_estimator)
        ...
        # ── Phase 2: main decision — optimized or baseline ─────────────────
            ...
            if safety_margin >= MIN_SAFE_HORIZON:
                ...
                # TODO: do dynamic safety margins need to be updated below at each step?
                else:
                    ...
                    verify_horizon = conflict_time - current_timestep
                    # pending_job, result = symbolic_step(tester, pending_job, chunk_size)
                    pending_job, result = symbolic_step(tester, pending_job, verify_horizon, budget.max_affordable_symbolic(), time_estimator)
