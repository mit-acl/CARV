...
def unicycle_mpc(model: do_mpc.model.Model,
    ...
    # obs_inflation: scalar added to every obstacle radius at solve time.
    obs_inflation = model.set_variable('_tvp', 'obs_inflation', shape=(1, 1))
    if nominal_tracking:
    ...
    if obstacles is None:
        ...
        mpc.set_nl_cons('obs1', (r1 + obs_inflation)**2 - dist_sq1, ub=0)

        cx2, cy2, r2 = -2.0, 1.5, 1.5
        dist_sq2 = (p[0] - cx2)**2 + (p[1] - cy2)**2
        mpc.set_nl_cons('obs2', (r2 + obs_inflation)**2 - dist_sq2, ub=0)
    else:
        # Circular obstacles: obs = np.array([cx, cy, r])
        # effective radius = r + obs_inflation (TVP).
        for i_obs, obs in enumerate(obstacles):
            cx, cy, r = float(obs[0]), float(obs[1]), float(obs[2])
            dist_sq = (p[0] - cx)**2 + (p[1] - cy)**2
            print(f"obs{i_obs}: center=({cx},{cy})  r={r}")
            mpc.set_nl_cons(f'obs{i_obs}_circle',
                            (r + obs_inflation)**2 - dist_sq, ub=0)

    # Single TVP function handles both obs_inflation and (optionally) omega_nom.
    mpc._obs_inflation_buf = np.zeros(1)   # set before each solve
    tvp_template = mpc.get_tvp_template()

    if nominal_tracking:
        def tvp_fun(t_now):
            for k in range(n_horizon + 1):
                tvp_template['_tvp', k, 'obs_inflation'] = mpc._obs_inflation_buf[0]
