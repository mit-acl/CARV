"""
Animation for alg6 — dynamic symbolic horizon with single-timestep enforcement.

Same visual style as anim_alg5. Key behavioural differences vs alg5:
  - symbolic_step takes timestep_length to decide if chunk fits in one shot
  - dynamic_symbolic_horizon grows with safety margin (max(MAX, margin - BUFFER))
  - optimized_step symbolic branch drops budget cap from span
  - dynamic_symbolic_horizon updated inside optimizer loop each iteration
...
from alg6_optimized_w_varied_symbolic_horizon import concrete_scan, symbolic_step, VerificationTask, _get_volume, get_dynamic_symbolic_horizon
import numpy as np
...
def optimized_step_anim(frames, tester, validated_until, max_time, budget,
    ...
    if method == "concrete":
        ...
        actual_k  = min(full_span, max_symbolic_horizon)   # budget cap removed
...
out_path = "alg6_optimization.gif"
